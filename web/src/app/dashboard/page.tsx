...
const Dashboard = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetUser({ token: userInfo?.token || '', id: userInfo?.id || '' })
  ...
  useEffect(() => {
    if (user && userInfo) {
      ...
      if (!user.emailVerified) router.push('/auth/activate')
    }
  }, [user])
  ...
  if (isError || !user) {
    return <Error message="Erro ao carregar os dados do usuário" />
  }
        ...
        <ScrollArea.Root className="w-full h-[85%]">
          <ScrollArea.Viewport className="w-full h-full py-3 px-4">
            <div>
              ...
              <UserImages token={userInfo.token} userId={userInfo.id} preview />
            </div>
            <div>
              <div className="flex flex-col items-center justify-between px-4 py-2 w-full">
                <Title className="text-start text-xl">Últimos posts</Title>
              </div>
              <UserPosts token={userInfo.token} userId={userInfo.id} preview />
            </div>
            <div>
              <div className="flex flex-col items-center justify-between px-4 py-2 w-full">
              ...
              <ListUsers token={userInfo.token} userId={userInfo.id} preview />
            </div>
          </ScrollArea.Viewport>
